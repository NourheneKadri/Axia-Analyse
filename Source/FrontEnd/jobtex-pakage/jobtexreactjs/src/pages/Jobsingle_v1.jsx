...
const deadline = moment(jobDetails.deadlineTimestamp);
  ...
  // Afficher le message en fonction du nombre de jours restants
  let message = "";
  if (daysLeft > 0) {
    message = `${daysLeft} days left to apply`;  // Afficher le nombre de jours restants
  } else if (daysLeft === 0) {
    message = "Today is the last day to apply!";
  } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5259/api/JobOfferCandidancy", {
        method: "POST",
        body: formData, // Ne spécifiez pas Content-Type
  ...
  const handleFileChange = (e) => {
    ...
    setApplication({
      ...application,
      CV: file,  // Le fichier CV téléchargé
    });
  };
  return (
    <>
                  ...
                  <Link to="#  "  className="category">
                    ...
                    <ul className="job-info">
                      <li>
                        ...
                        <span>{jobDetails.adress}</span>
                      </li>
                      <li>
                        <span className="icon-calendar"></span>
                        <span>{moment(jobDetails.timestamp).fromNow()}</span>
                      </li>
                    </ul>
                    <ul className="tags">
                      <li>
