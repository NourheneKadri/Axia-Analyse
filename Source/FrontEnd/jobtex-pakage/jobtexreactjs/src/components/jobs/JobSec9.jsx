...
function JobSec2(props) {
  ...
  useEffect(() => {
    const user = Authentification.getStoredUser(); // Get current user
    const fetchData = async () => {
      try {
        const response = await JobOfferServices.getActiveJobOffers(companyId); // ← ICI il faut "await"
  ...
  return (
    <section className={`inner-jobs-section ${className}`}>
      <div className="tf-container" style={{ paddingBottom: "38px" }}>
        <div className="job-search-form inner-form-map st1"  style={{ paddingTop: "38px" }}>
